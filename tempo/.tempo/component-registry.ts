...
const createLoaders = () => ({
    "tempo/designs/canvases/home/index.canvas.tsx": createCanvasLoader(() => import("../designs/canvases/home/index.canvas.tsx"), "../designs/canvases/home/index.canvas.tsx"),
    "tempo/designs/canvases/template-canvas/index.canvas.tsx": createCanvasLoader(() => import("../designs/canvases/template-canvas/index.canvas.tsx"), "../designs/canvases/template-canvas/index.canvas.tsx"),
});

	const registerCurrentLoaders = (changedSpecifiers?: readonly (string | { specifier: string; changedExports?: readonly string[] })[]) => registerCanvasLoaders(createLoaders(), changedSpecifiers);
  ...
  "../designs/canvases/template-canvas/index.canvas.tsx",
	...
	const canvasHotMetadata = {
	  "../designs/canvases/home/index.canvas.tsx": {"importPath":"tempo/designs/canvases/home/index.canvas.tsx","ambientHash":"99a1d9406d692f47","exports":{"Home":"215b1e5a4ea7b312"}},
  "../designs/canvases/template-canvas/index.canvas.tsx": {"importPath":"tempo/designs/canvases/template-canvas/index.canvas.tsx","ambientHash":"a4aa0b6594df0031","exports":{"Example":"47f1da036888cdd5"}},
} as const;
  ...
  const withoutFs = clean.startsWith("/@fs/") ? clean.slice("/@fs".length) : clean;
...
const hotPathMatchesSpecifier = (specifier: string, changedPath: string): boolean => {
  ...
  return relativeSpecifier.length > 0 && normalizedChangedPath.endsWith("/" + relativeSpecifier);
	};
	const hotPathCanAffectSpecifier = (specifier: string, changedPath: string): boolean => {
	  ...
	  if (normalizedSpecifierDir && normalizedChangedPath.startsWith(normalizedSpecifierDir + "/")) {
	    return true;
	  }
	  const relativeSpecifier = stripHotRelativeSegments(specifier);
	  ...
	  return relativeSpecifierDir.length > 0 && (
	    relativeChangedPath.startsWith(relativeSpecifierDir + "/") ||
	    normalizedChangedPath.includes("/" + relativeSpecifierDir + "/")
	  );
	};
	const hot = (import.meta as { hot?: { data?: Record<string, unknown>; accept: (...args: unknown[]) => void; on?: (event: string, callback: (payload: unknown) => void) => void } }).hot;
	const getPreviousCanvasHotMetadata = (): Record<string, CanvasHotMetadataEntry> | undefined => {
	  return hot?.data?.tempoCanvasHotMetadata && typeof hot.data.tempoCanvasHotMetadata === "object"
	    ? hot.data.tempoCanvasHotMetadata as Record<string, CanvasHotMetadataEntry>
	    : undefined;
	};
	const changedExportsForSpecifier = (specifier: string): readonly string[] | undefined => {
	  const current = (canvasHotMetadata as Record<string, CanvasHotMetadataEntry>)[specifier];
	  const previous = getPreviousCanvasHotMetadata()?.[specifier];
	  if (!current || !previous || current.parseFailed || previous.parseFailed) return undefined;
	...
	const metadataChangedForSpecifier = (specifier: string): boolean => {
	  const current = (canvasHotMetadata as Record<string, CanvasHotMetadataEntry>)[specifier];
	  const previous = getPreviousCanvasHotMetadata()?.[specifier];
	  if (!current || !previous || current.parseFailed || previous.parseFailed) return true;
	...
	const toCanvasSpecifierChanges = (specifiers: readonly string[]): readonly { specifier: string; changedExports?: readonly string[] }[] => {
	...
	const rememberChangedCanvasSpecifiers = (specifiers: readonly string[]): void => {
	  if (!hot || specifiers.length === 0) return;
	  const existing = Array.isArray(hot.data?.tempoChangedCanvasSpecifiers)
    ? hot.data.tempoChangedCanvasSpecifiers.filter((value): value is string => typeof value === "string")
    : [];
	  hot.data ??= {};
	  hot.data.tempoChangedCanvasSpecifiers = [...new Set([...existing, ...specifiers])];
	};
	const collectCanvasSpecifier = (
  value: unknown,
  changed: Set<string>,
): void => {
...
const readCanvasHotUpdate = (payload: unknown): { changed: Set<string> } => {
  ...
  const collectUpdate = (update: unknown): void => {
    ...
    const candidate = update as { path?: unknown; acceptedPath?: unknown; id?: unknown };
  ...
  const maybeUpdates = (payload as { updates?: unknown } | null | undefined)?.updates;
...
const takeChangedCanvasSpecifiers = (): readonly string[] | undefined => {
  ...
  if (hot?.data) {
	...
	const changedCanvasSpecifiers = takeChangedCanvasSpecifiers() ?? collectMetadataChangedCanvasSpecifiers();
	registerCurrentLoaders(wasHotUpdate ? toCanvasSpecifierChanges(changedCanvasSpecifiers) : undefined);
	if (hot) {
	  hot.data ??= {};
    ...
    rememberChangedCanvasSpecifiers(["../designs/canvases/home/index.canvas.tsx"]);
  });
  hot.accept("../designs/canvases/template-canvas/index.canvas.tsx", () => {
    rememberChangedCanvasSpecifiers(["../designs/canvases/template-canvas/index.canvas.tsx"]);
  });
  hot.accept();
}
