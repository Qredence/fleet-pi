...
- Hard-cutover auth migration: users re-authenticate on Neon Managed Auth; best-effort email remap via `pnpm remap-auth-user-ids -- --file=remap.csv` (owner migration URL). Orphan mirror cleanup: `pnpm quarantine-orphan-sessions`. Neon user-scoped RLS applies to `pi_*` mirror tables via `app.current_user_id`; Better Auth public tables (`user`, `session`, `account`, `verification`) enable RLS with a `fleet_pi_app`-only policy (not user-scoped)—`pnpm auth:migrate` chains `auth-post-migrate.ts` for that plus Data API revokes. Chat migrations also revoke Neon Data API `authenticated`/`anonymous` table grants on `pi_*` and EXECUTE on ownership SECURITY DEFINER probes (`fleet_pi_check_session_owner`, `fleet_pi_lookup_session_id_by_file`)—only `fleet_pi_app` may call them. On Vercel, per-user Pi project settings (skills/packages/models) live in `pi_user_settings` JSONB and are hydrated into live `SettingsManager` instances because `.pi/settings.json` is not writable.
