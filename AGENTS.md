...
- Remap `pi_*` ownership after Neon Managed Auth cutover with `pnpm --filter web remap-auth-user-ids -- --file=remap.csv` (`email,newUserId` CSV; `--dry-run` first).
...
- Better Auth on Vercel requires `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, and `FLEET_PI_AUTH_DATABASE_URL` when **not** using Neon Managed Auth. With Neon Managed Auth (`NEON_AUTH_BASE_URL` or Vercel↔Neon `NEON_AUTH_URL`), use `NEON_AUTH_COOKIE_SECRET`, `VITE_NEON_AUTH_URL`, `NEON_AUTH_JWKS_URL`, and consolidate `FLEET_PI_AUTH_DATABASE_URL` with `FLEET_PI_CHAT_DATABASE_URL` on `fleet-pi-neon` (also linked to Vercel + GitHub). Local anonymous/SQLite Better Auth applies only when neither Neon Auth URL is set.
...
- Settings dialog should follow shadcn sidebar-13: icon+label nav only (no subtitles), scrollable main pane, and flat `ItemRow` lists without nested SectionSurface wrappers; Providers lists only configured/active providers; OpenAI Chat Completions requires API key + base URL + model ID; LLM Models should discover models from configured providers (for example provider `/models`) into a curated enable/remove list, grouped by provider with an All/Enabled filter.
