...
- Hard-cutover auth migration: users re-authenticate on Neon Managed Auth; best-effort email remap via `pnpm remap-auth-user-ids -- --file=remap.csv` (owner migration URL). Orphan mirror cleanup: `pnpm quarantine-orphan-sessions`. Identity lives in `neon_auth` (Managed Auth); `pnpm auth:migrate` drops legacy `public` Better Auth tables when `neon_auth` exists. Do not enable or alter RLS on `neon_auth` (Neon-owned; no Data API/`fleet_pi_app` schema USAGE). Tenant isolation is `public.pi_*` only, via InitPlan-safe `(SELECT current_setting('app.current_user_id', true))`. Legacy self-hosted Better Auth (no `neon_auth`) keeps public auth tables with `fleet_pi_app`-only RLS. Chat migrations revoke Neon Data API `authenticated`/`anonymous` grants on `pi_*` and EXECUTE on ownership SECURITY DEFINER probes (`fleet_pi_check_session_owner`, `fleet_pi_lookup_session_id_by_file`)—only `fleet_pi_app` may call them. On Vercel, per-user Pi project settings (skills/packages/models) live in `pi_user_settings` JSONB and are hydrated into live `SettingsManager` instances because `.pi/settings.json` is not writable.
