...
- The Pi resources browser in `apps/web/src/routes/index.tsx` fetches `/api/chat/resources` and surfaces discovered skills, prompts, extensions, themes, context files, and diagnostics. Desktop open state is a resizable right-side canvas; mobile open state is a compact overlay.
...
- Open the Pi resources browser and verify the starter skills, npm package resources, vendored extensions, and `project-inventory.ts` appear. On desktop, verify the resources canvas docks to the right and resizes; on mobile, verify it opens as an overlay. If both project-local and global `agent-elements` skills are installed, the resources diagnostics may include that expected name collision.
