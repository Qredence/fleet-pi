...
- Remap `pi_*` ownership after Neon Managed Auth cutover with `pnpm remap-auth-user-ids -- --file=remap.csv` (`email,oldUserId,newUserId` or legacy `email,newUserId` when `public."user"` still exists; `--dry-run` first).
...
- Better Auth on Vercel requires `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, and `FLEET_PI_AUTH_DATABASE_URL` when **not** using Neon Managed Auth. With Neon Managed Auth (`NEON_AUTH_BASE_URL` or Vercel↔Neon `NEON_AUTH_URL`), use `NEON_AUTH_COOKIE_SECRET`, `VITE_NEON_AUTH_URL`, `NEON_AUTH_JWKS_URL`, `NEON_AUTH_ISSUER` (required so bearer JWTs fail closed), and consolidate `FLEET_PI_AUTH_DATABASE_URL` with `FLEET_PI_CHAT_DATABASE_URL` on `fleet-pi-neon` (also linked to Vercel + GitHub). Under Managed Auth, `BETTER_AUTH_SECRET` is still required for BYOK provider AES-GCM encryption (cookie gate prefers `NEON_AUTH_COOKIE_SECRET`). Local anonymous/SQLite Better Auth applies only when neither Neon Auth URL is set.
...
- Prefer Neon Managed Auth cutover (`neon_auth` identity + `pi_*` tenancy RLS) over RLS-hardening legacy public Better Auth leftovers as a permanent posture; keep security/isolation audit notes gitignored (e.g. `.cursor/hooks/state/`), not under tracked `agent-workspace/artifacts/`.

## Learned Workspace Facts

- Repo-root `neon.ts` (`@neon/config`) enables Managed Auth and the `chat` Neon Function on `fleet-pi-neon` in `aws-us-east-2` (required for Neon beta services). Object Storage buckets are not declared in `neon.ts` (no local `AWS_*` env pull). Link the existing project with `neon link` (`.neon` branch pin is gitignored). `neon deploy` provisions services only—run `pnpm auth:migrate` / `pnpm chat:migrate` separately. Deployed chat runtime APIs stream from the Neon Function when `VITE_FLEET_PI_CHAT_RUNTIME_URL` is set; settings/providers/workspace stay on Vercel. `.github/workflows/neon_workflow.yml` creates/deletes Neon preview branches for PRs (needs repo `NEON_PROJECT_ID` + `NEON_API_KEY`).
- Hard-cutover auth migration: users re-authenticate on Neon Managed Auth; best-effort email remap via `pnpm remap-auth-user-ids -- --file=remap.csv` (owner migration URL). Orphan mirror cleanup: `pnpm quarantine-orphan-sessions`. Identity lives in `neon_auth` (Managed Auth); `pnpm auth:migrate` drops legacy `public` Better Auth tables when `neon_auth` exists—do not keep those leftovers as the long-term model. Keep Neon-owned identity tables private and do not create views that expose `neon_auth.user` to Data API roles. Fleet Pi follows Neon’s Managed Auth **server/Function** pattern (cookies + JWKS Bearer JWT + per-user scoping on `fleet_pi_app`); Neon Data API is a separate optional product and must stay **off**—do not enable it to “complete” Managed Auth. Neon user-scoped RLS applies to `pi_*` mirror tables via `app.current_user_id` (live DBs may also expose `fleet_pi_current_user_id()` wrapping `auth.user_id()` then the GUC); `FORCE ROW LEVEL SECURITY` is required on `pi_*` so owner connection strings cannot silently bypass isolation—runtime `FLEET_PI_CHAT_DATABASE_URL` must be non-owner `fleet_pi_app`. Better Auth tables (`user`, `session`, `account`, `verification`) and `neon_auth.*` must not use RLS—`pnpm auth:migrate` chains `auth-post-migrate.ts` to disable RLS and grant `fleet_pi_app` DML. Chat migrations revoke Neon Data API `authenticated`/`anonymous` table grants on `pi_*` and EXECUTE on ownership SECURITY DEFINER probes (`fleet_pi_check_session_owner`, `fleet_pi_lookup_session_id_by_file`)—only `fleet_pi_app` may call them. Keep Neon Data API **disabled** in `neon.ts` (`dataApi: false`) and in console (no public-schema grants). Do not re-apply `chat-postgres-data-api-auth` grant migrations for closed beta; re-assert with `20260723_revoke_data_api_pi_grants_again` if orphan Data API grant migrations reappear; `verify-deployment-readiness` fails when those roles still hold privileges on `pi_sessions` / `pi_user_providers` / `pi_user_settings`. Neon Managed Auth open signup is closed-beta only until restricted signups exist (see `docs/runbooks/deployment-release-gate.md`). On Vercel, per-user Pi project settings (skills/packages/models) live in `pi_user_settings` JSONB and are hydrated into live `SettingsManager` instances because `.pi/settings.json` is not writable.
...
- On Vercel (`VERCEL=1`), session-scoped chat endpoints require auth (Neon Managed Auth when `NEON_AUTH_BASE_URL`/`NEON_AUTH_URL` is set, otherwise Better Auth): `/api/chat`, `/api/chat/session`, `/api/chat/resume`, `/api/chat/sessions`, `/api/chat/new`, `/api/chat/abort`, `/api/chat/question`, `/api/chat/runs`, `/api/chat/run`, `/api/chat/provenance`, `PATCH /api/chat/settings`, plus catalog `/api/chat/models`, `/api/chat/models/discover`, `/api/chat/resources`, and `/api/chat/commands`. Workspace APIs (`/api/workspace/tree`, `/file`, `/item(s)`, `/search`, `/reindex`, `/health`) use the same `withAuthenticatedChatRequest` gate when chat auth is required and resolve Daytona via `resolveWorkspaceContext` when the user has BYOK. Local anonymous chat/workspace applies only when neither Neon Auth URL is set. `auth-mode.ts` must stay client-safe—server chat auth (`isChatAuthRequired`, `chat-auth-surface`, `node:async_hooks`) lives in `chat-api-auth.ts`. Neon Auth clients must mint JWTs via `authClient.token()` with `credentials: "include"` (not Better Auth `getJWTToken()`). `/api/chat/providers` uses `getChatAuthSession`/`withAuthenticatedChatRequest` like settings. Mirror ownership fail-closed (`shouldFailClosedOnMirrorError` / `requiresAuthenticatedMirrorOwner`) lives in `deployment/trust-zone.ts` (not client-safe `environment.ts`) on Vercel, Neon Managed Auth, Neon Function surface, or `FLEET_PI_CHAT_RUNTIME_REQUIRE_AUTH`; probes use `fleet_pi_check_session_owner` (SECURITY DEFINER) so RLS cannot hide foreign-owned rows—run `pnpm chat:migrate` after deploy when ownership or Data API revoke migrations change.
