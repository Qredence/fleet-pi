...
- `apps/desktop` is the installable Electron shell. `apps/desktop/src/main.ts` owns the BrowserWindow lifecycle, native menus, project picker, and Electron security policy. `apps/desktop/src/desktop-state.ts` persists recent projects, the active project, and per-workspace session metadata under Electron `userData`.
...
- `apps/web/src/lib/desktop/server.ts` resolves the active runtime context. In browser mode it falls back to this repo root. In Electron mode it requires the desktop auth token, reads the active project from Electron state, and returns both `projectRoot` and `workspaceRoot`.
...
- Pi sessions are persistent JSONL sessions. Browser mode stores only Pi session metadata in localStorage and hydrates visible messages from the Pi session file after refresh. Electron desktop mode stores the active session mapping in Electron `userData` per selected project workspace.
...
- Open the Pi resources browser and verify the starter skills, npm package resources, vendored extensions, `project-inventory.ts`, and `workspace-index.ts` appear. On desktop, verify the resources canvas docks to the right and resizes; on mobile, verify it opens as an overlay. Switch to the Workspace tab, verify `agent-workspace` plus representative seeded files appear, and click a Markdown file to verify the preview panel renders its content. Switch to Configurations and verify the UI-first rows plus Light/Dark/System theme control. If both project-local and global `agent-elements` skills are installed, the resources diagnostics may include that expected name collision.
- Start the Electron dev flow with the root desktop dev script and verify the app starts at an empty project-picker state before any project is selected.
- In the desktop app, open an existing local project directory and verify `<project>/agent-workspace/` is created, the workspace label updates, and the native Recent Projects menu is populated.
