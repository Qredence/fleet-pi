...
- Daytona provisions per-user sandboxes via `resolveUserSandboxContext({ userId, surface })` (`apps/web/src/lib/daytona/resolve-user-sandbox-context.ts`); persistent volume mirrors `agent-workspace/` (sparse-seeded from git, not the full repo), with per-user provider API keys and OAuth subscription tokens injected into the sandbox environment. Deepen by unifying auth→sandbox→workspaceRoot mount/lifecycle first; sandbox-backed tool re-registration is a follow-up behind the same interface (toward the official Daytona Pi extension model).
