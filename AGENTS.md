...
- Hard-cutover auth migration: users re-authenticate on Neon Managed Auth; best-effort email remap via `pnpm remap-auth-user-ids -- --file=remap.csv` (owner migration URL). Orphan mirror cleanup: `pnpm quarantine-orphan-sessions`. Neon user-scoped RLS applies to `pi_*` mirror tables via `app.current_user_id` (live DBs may also expose `fleet_pi_current_user_id()` wrapping `auth.user_id()` then the GUC); `FORCE ROW LEVEL SECURITY` is required on `pi_*` so owner connection strings cannot silently bypass isolation—runtime `FLEET_PI_CHAT_DATABASE_URL` must be non-owner `fleet_pi_app`. Better Auth tables (`user`, `session`, `account`, `verification`) and `neon_auth.*` must not use RLS—`pnpm auth:migrate` chains `auth-post-migrate.ts` to disable RLS and grant `fleet_pi_app` DML. Chat migrations revoke Neon Data API `authenticated`/`anonymous` table grants on `pi_*` and EXECUTE on ownership SECURITY DEFINER probes (`fleet_pi_check_session_owner`, `fleet_pi_lookup_session_id_by_file`)—only `fleet_pi_app` may call them. Keep Neon Data API **disabled** in console (no public-schema grants). Re-assert with `20260723_revoke_data_api_pi_grants_again` if orphan Data API grant migrations reappear; `verify-deployment-readiness` fails when those roles still hold privileges on `pi_sessions` / `pi_user_providers` / `pi_user_settings`. Neon Managed Auth open signup is closed-beta only until restricted signups exist (see `docs/runbooks/deployment-release-gate.md`). On Vercel, per-user Pi project settings (skills/packages/models) live in `pi_user_settings` JSONB and are hydrated into live `SettingsManager` instances because `.pi/settings.json` is not writable.
