...
- Daytona provisions per-user sandboxes via `resolveUserSandboxContext({ userId, surface })` (`apps/web/src/lib/daytona/resolve-user-sandbox-context.ts`); a durable per-user volume mounts `agent-workspace/` only (sparse-seeded from git, never the full repo; seed must not overwrite user changes on reopen). Inject per-user provider API keys and OAuth subscription tokens into the sandbox env. Unify auth→sandbox→workspaceRoot mount/lifecycle first; then re-register bash/file/grep tools to run in-sandbox with agent-workspace cwd (toward `@daytona/pi`).
