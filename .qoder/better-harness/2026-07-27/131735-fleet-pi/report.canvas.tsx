...
function mergeCanvasRows(hostRows, canvasRows) {
  ...
  return (Array.isArray(hostRows) ? hostRows : []).map((row) => ({
    ...detailById.get(row?.id),
    ...row,
  }))
}
...
function mergeCanvasReport(host, detail) {
  ...
  return {
    summary: {
      ...(detail?.summary ?? {}),
      ...summary,
      atAGlance: mergeCanvasObjects(
        detail?.summary?.atAGlance,
        summary.atAGlance
      ),
...
const DIMENSION_SUMMARY_EXAMPLE =
  "Example: project guidance makes the main workflow clear, but ownership for cross-cutting changes is not documented."
...
function openingStrengths() {
  const explicit = list(report.summary?.strengths)
    .map(textValue)
    .filter(Boolean)
  return explicit.length
    ? explicit.slice(0, 3)
    : [
        "Reviewed project signals are organized into dimensions and issue findings.",
      ]
}
...
function averageScore(dimensions) {
  ...
  return Math.round(
    dimensions.reduce((sum, row) => sum + clampScore(row.score), 0) /
      dimensions.length
  )
}
...
function splitFluencyTooltipReason(value) {
  ...
  const chunks = []
  const limits = /[\u3400-\u9fff]/.test(remaining)
    ? [20, 20, 20, 20]
    : [34, 30, 30, 30]
...
function dimensionFluencyStages(dimensions) {
  return dimensions.map((row) => {
    ...
    return {
      ...
      score,
      ...(usesGenericBand
        ? { status: stageStatus(score), blocker: score <= 20 }
        : {}),
    }
  })
}
...
function practiceDescription(surface) {
  const descriptions = {
    Rules: [
      "Standing project guidance and task-routing instructions.",
      "项目常驻指引与任务路由说明。",
    ],
    Skills: [
      "Reusable agent workflows available to the project.",
      "项目可用的可复用 Agent 工作流。",
    ],
    "Custom Agents": [
      "Specialized agent profiles available for delegated work.",
      "可用于委派工作的专用 Agent 配置。",
    ],
    Hooks: [
      "Lifecycle automation around agent and delivery events.",
      "围绕 Agent 与交付事件的生命周期自动化。",
    ],
    MCP: [
      "External tools and resources exposed through MCP.",
      "通过 MCP 暴露的外部工具与资源。",
    ],
    Commands: [
      "Named command entry points for repeatable agent work.",
      "可重复 Agent 工作的命令入口。",
    ],
    Workflows: [
      "Reusable multi-step project workflows.",
      "可复用的多步骤项目工作流。",
    ],
    Plugins: [
      "Installed packages that contribute agent capabilities.",
      "提供 Agent 能力的已安装插件。",
    ],
    "Session Insights": [
      "Task-session evidence available for report analysis.",
      "可用于报告分析的任务会话证据。",
    ],
    Memories: [
      "Representative project or global Memory note files.",
      "项目级或全局 Memory 的代表性笔记文件。",
    ],
  }
  const copy = descriptions[surface] ?? [
    "Recorded agent capability sources.",
    "已记录的 Agent 能力来源。",
  ]
  ...
  return (
    <Stack gap={4}>
      {preview.map((path, index) => (
        <Text key={`${path}-${index}`} size="sm" tone="tertiary" truncate>
          {path}
        </Text>
      ))}
      {remaining.length ? (
        <CollapsibleSection
          ...
          title={
            <Text size="sm" weight="semibold">
              {taskLoopCopy(
                `View ${remaining.length} more locations`,
                `查看其余 ${remaining.length} 个位置`
              )}
            </Text>
          }
          ...
          {scopes.length || paths.length ? (
            <Stack gap={6} style={{ marginTop: "auto" }}>
              {scopes.length ? (
                <Row gap={5} wrap>
                  {scopes.map((scope) => (
                    <Tag key={scope} size="sm" tone="info">
                      {scope}
                    </Tag>
                  ))}
                </Row>
              ...
              {paths.length ? (
                <Stack gap={4}>
                  <Text size="sm" weight="semibold" tone="tertiary">
      ...
      {dimensions.map((row) => {
        ...
        return (
          <Card key={row.id} size="sm">
            <CardHeader
              title={
                <Text weight="semibold">{taskLoopDimensionLabel(row.id)}</Text>
              }
  ...
  return (
    <Card size="sm">
      <CardHeader
        ...
        trailing={
          <Tag tone={severityTone(row.severity)}>
      ...
      <CardBody>
        <Stack gap={8}>
          <Row gap={8} align="center" wrap>
            {list(row.dimensionRefs)
              .slice(0, 1)
              .map((ref) => (
                <Tag key={ref} tone="neutral">
  ...
  return (
    <Stack gap={8}>
      <Row justify="space-between" align="center" wrap gap={8}>
        ...
        {surfaces.length ? (
          <Tag tone="neutral">{surfaces.length} surfaces</Tag>
        ) : null}
      </Row>
      {rows.length ? (
        <Grid columns={3} minColumnWidth={260} gap={10} align="stretch">
          {rows.map((row, index) => (
            <PracticeSourceCard key={row.surface ?? index} row={row} />
          ))}
        </Grid>
      ) : (
...
function usesChineseReaderCopy() {
  ...
  const readerSample = [
    ...
    ...list(report.findings)
      .slice(0, 3)
      .flatMap((row) => [row?.title, row?.reason, row?.reader]),
  ]
    .map(textValue)
    .join(" ")
...
function learningStateLabel(value) {
  ...
  return labels
    ? taskLoopCopy(labels[0], labels[1])
    : taskLoopCopy("Not observed", "未观察到")
}
...
function usageActivityMatrix(activity) {
  ...
  const columns = Array.from({ length: weekCount }, (_, index) => {
    ...
    const previous =
      index > 0
        ? new Date(start.getTime() + (index - 1) * 7 * 86_400_000)
        : null
  ...
  sourceDates.forEach((date, index) => {
    ...
    const week = Math.floor(
      (parsed.getTime() - start.getTime()) / (7 * 86_400_000)
    )
    ...
    values[parsed.getUTCDay()][week] = {
      ...
      value: active,
      ariaLabel: taskLoopCopy(
        `${date}: ${formatActivityMinutes(active)}`,
        `${date}：${formatActivityMinutes(active)}`
      ),
    }
  })
...
function usageChartWindow(activity) {
  ...
  return {
    offset,
    categories: dates.slice(offset).map((date) => String(date).slice(5)),
  }
}
...
function visibleUsageSeries(series, offset, limit = 5) {
  ...
  const primary = named
    .slice(0, limit)
    .map((row) => ({
      name: usageSeriesLabel(row.name),
      data: list(row.daily).slice(offset),
      total: Number(row.total ?? 0),
    }))
  const remainder = [
    ...named.slice(limit),
    ...rows.filter((row) => row?.name === "Other"),
  ]
  if (remainder.length > 0) {
    const length =
      primary[0]?.data.length ?? list(remainder[0]?.daily).slice(offset).length
    primary.push({
      name: "Other",
      ...
      data: Array.from({ length }, (_, index) =>
        remainder.reduce(
          (sum, row) =>
            sum + Number(list(row?.daily).slice(offset)[index] ?? 0),
          0
        )
      ),
    })
...
function formatUsageNumber(value) {
  return Math.round(Number(value ?? 0)).toLocaleString(
    usesChineseReaderCopy() ? "zh-CN" : "en-US"
  )
}

function usageSeriesLabel(value) {
  if (value === "Unknown model")
    return taskLoopCopy("Unattributed model", "未归属模型")
  if (value === "Unknown Skill")
    return taskLoopCopy("Unattributed Skill", "未归属 Skill")
  return value
}
...
function confidenceLabel(value) {
  ...
  return labels
    ? taskLoopCopy(labels[0], labels[1])
    : taskLoopCopy("Confidence not recorded", "未记录可信度")
}
...
function evidenceReferenceLabel(item) {
  return (
    ...
    textValue(item?.id) ||
    taskLoopCopy("Unnamed evidence", "未命名证据")
  )
}
  ...
  return (
    ...
    >
      {items.map((item, index) => (
        <Stack
          key={`${item?.group ?? item?.kind}-${item?.id ?? index}`}
          gap={3}
        >
...
function practiceSurfaceGlyph(surface) {
  return (
    { Rules: "R", Skills: "S", "Custom Agents": "A", Hooks: "H", MCP: "M" }[
      surface
    ] ??
    textValue(surface).slice(0, 1).toUpperCase() ??
    "?"
  )
}
...
function PracticeSurfaceIcon({ row }) {
  return (
    <Tag size="sm" tone="neutral">
      {practiceSurfaceGlyph(row?.surface)}
    </Tag>
  )
}
  ...
  return (
    <Stack gap={8}>
      ...
      {overview ? (
        <Text
          tone="secondary"
          style={{ ...taskLoopReaderCopyStyle, lineHeight: 1.5 }}
        >
          {overview}
        </Text>
      ) : null}
      <Row gap={10} align="center" wrap>
        <Text size="sm" tone="secondary">
          {taskLoopCopy(
            `${findings.length} prioritized improvements`,
            `${findings.length} 项优先优化`
          )}
        </Text>
        {sources.length ? (
          <Text size="sm" tone="secondary">
            {taskLoopCopy(
              `${sources.length} practice source types`,
              `${sources.length} 类实践来源`
            )}
          </Text>
        ) : null}
      </Row>
...
function practiceSourceCell(row) {
  ...
  return firstPath ? (
    <Text size="sm" tone="secondary" style={{ overflowWrap: "anywhere" }}>
      {firstPath}
    </Text>
  ) : (
    <Text size="sm" tone="tertiary">
    ...
    },
    {
      ...
      render: (row) => (
        <Stack gap={4}>
          <Text size="sm" weight="semibold">
            {taskLoopCopy(
              `${practiceCount(row)} sources`,
              `${practiceCount(row)} 个来源`
            )}
          </Text>
...
function TaskLoopPracticeTable({ rows = practiceRows() }) {
  if (rows.length === 0) {
    return (
      <Text tone="secondary">
        {taskLoopCopy("No Agent assets recorded.", "未记录 Agent 工程资产。")}
      </Text>
    )
  }
  return (
    <Table
      ...
      emptyText={taskLoopCopy(
        "No Agent asset coverage recorded",
        "未记录 Agent 工程资产覆盖"
      )}
    />
  )
...
function TaskLoopActivityHeatmap({ activity }) {
  ...
  if (!matrix)
    return (
      <Text tone="secondary">
        {taskLoopCopy(
          "No dated session activity was observed.",
          "没有观察到带日期的会话活动。"
        )}
      </Text>
    )
    ...
    <Stack gap={6}>
      ...
      <RiskHeatmap
        ...
        colorTemplate={{
          ...
          low: {
            background: "rgba(64, 166, 103, 0.22)",
            border: "transparent",
          },
          medium: {
            background: "rgba(54, 158, 94, 0.42)",
            border: "transparent",
          },
          high: {
            background: "rgba(38, 139, 78, 0.66)",
            border: "transparent",
          },
          critical: {
            background: "rgba(24, 115, 63, 0.9)",
            border: "transparent",
          },
        }}
        maxHeight={190}
...
function UsageStatRow({ label, value }) {
  ...
  return (
    <Row justify="space-between" align="start" gap={12} style={{ minWidth: 0 }}>
      <Text size="sm" tone="secondary" style={{ flexShrink: 0 }}>
        {label}
      </Text>
      <Text
        size="sm"
        weight="semibold"
        style={{ minWidth: 0, overflowWrap: "anywhere", textAlign: "right" }}
      >
        {value}
      </Text>
    </Row>
...
function UsageRankList({ series, limit = 5 }) {
  ...
  if (!rows.length)
    return (
      <Text size="sm" tone="secondary">
        {taskLoopCopy("No usage observed.", "未观察到用量。")}
      </Text>
    )
  return (
    <Stack gap={7}>
      {rows.map((row, index) => (
        ...
        >
          <Row gap={8} align="center" style={{ minWidth: 0 }}>
            <Text size="sm" tone="tertiary" style={{ width: 18 }}>
              {index + 1}
            </Text>
...
function taskLoopModelUsageColumns() {
  return [
    ...
    },
    {
      ...
      align: "right",
      render: (row) => (
        <Text size="sm">{formatUsageNumber(row.responseCount)}</Text>
      ),
    },
    {
      ...
      align: "right",
      render: (row) => (
        <Text size="sm">{formatUsageNumber(row.usageFieldObservedCount)}</Text>
      ),
    },
    {
      ...
      align: "right",
      render: (row) => (
        <Text size="sm">{formatUsageNumber(row.nonZeroUsageCount)}</Text>
      ),
    },
  ]
}
...
function TaskLoopLongSessionReview({ usage }) {
  ...
  const longestActiveMinutes =
    usage.longSessions?.longestActiveMinutes ??
    Math.max(...samples.map((sample) => Number(sample.activeMinutes ?? 0)))
  return (
    <Stack gap={14}>
      <Row justify="space-between" align="start" wrap gap={12}>
        <Stack gap={5} style={{ flex: "1 1 620px", minWidth: 0 }}>
          <Row gap={8} align="center" wrap>
            <H2 style={{ fontSize: 18, lineHeight: 1.35 }}>
              {taskLoopCopy(
                `${pendingCount} long sessions need review`,
                `${pendingCount} 个长会话待复核`
              )}
            </H2>
      ...
      <Stack gap={0}>
        {samples.map((sample, index) => {
          ...
          const roleLabel =
            sample.role === "user-thread-candidate"
            ...
            <Stack key={sample.alias} gap={0}>
              ...
              >
                <Row
                  gap={12}
                  align="start"
                  style={{ flex: "1 1 540px", minWidth: 0 }}
                >
                  <Tag size="sm" tone="neutral">
                    {sample.alias}
                  </Tag>
                ...
                <Tag size="sm" tone="warning">
                  {formatUsageNumber(failureCount)}{" "}
                  {taskLoopCopy("failures", "失败事件")}
                </Tag>
              </Row>
...
function TaskLoopProjectUsage({ activity, usage }) {
  ...
  const activeMinutes = list(activity?.sessions?.activeMinutes).reduce(
  ...
  const analyzedSessions = census
    ...
    : activity
      ? formatUsageNumber(activity.sessions?.total)
      : null
  return (
    <Stack gap={12}>
      ...
      >
        <Stack gap={9}>
          ...
          <UsageStatRow
            label={taskLoopCopy("Sessions analyzed", "已分析会话")}
            value={analyzedSessions}
          />
          {activity ? (
            <UsageStatRow
              label={taskLoopCopy("Estimated active minutes", "估算活跃分钟数")}
              value={formatUsageNumber(activeMinutes)}
            />
          ) : null}
          {activity ? (
            <UsageStatRow
              label={taskLoopCopy("Skills used", "Skill 使用")}
              value={formatUsageNumber(skillUses)}
            />
          ) : null}
          {longSessions ? (
            <UsageStatRow
              label={taskLoopCopy(
                "Estimated active-long sessions",
                "估算活跃长会话"
              )}
              value={formatUsageNumber(longSessions.activeCount)}
            />
          ) : null}
        </Stack>
        <Stack gap={9}>
...
function TaskLoopUsageMethodology({ usage }) {
  ...
  const hasCoverage =
    census || Object.keys(taskSelection).length || roles || accounting
  ...
  return (
    <Stack gap={12}>
      ...
      {roles || accounting ? (
        <Grid
          columns="repeat(auto-fit, minmax(min(100%, 240px), 1fr))"
          gap={20}
          align="start"
        >
          {roles ? (
            <Stack gap={7}>
              ...
              <UsageStatRow
                label={taskLoopCopy("User-thread candidates", "用户任务候选")}
                value={formatUsageNumber(roles.userThreadCandidateCount)}
              />
              <UsageStatRow
                label={taskLoopCopy("Child-agent candidates", "子 Agent 候选")}
                value={formatUsageNumber(roles.childAgentCandidateCount)}
              />
            </Stack>
          ) : null}
          {accounting ? (
            <Stack gap={7}>
              ...
              <UsageStatRow
                label={taskLoopCopy("Responses", "响应数")}
                value={formatUsageNumber(accounting.responseCount)}
              />
              <UsageStatRow
                label={taskLoopCopy(
                  "Model-attributed responses",
                  "已归属模型响应"
                )}
                value={formatUsageNumber(
                  accounting.modelAttributedResponseCount
                )}
              />
              <UsageStatRow
                label={taskLoopCopy("Usage fields observed", "观察到用量字段")}
                value={formatUsageNumber(accounting.usageFieldObservedCount)}
              />
              <UsageStatRow
                label={taskLoopCopy("Non-zero usage records", "非零用量记录")}
                value={formatUsageNumber(accounting.nonZeroUsageCount)}
              />
              <UsageStatRow
                label={taskLoopCopy(
                  "Exact credits available",
                  "可精确计算 credit"
                )}
                value={
                  accounting.exactCreditsAvailable
                    ? taskLoopCopy("Yes", "是")
                    : taskLoopCopy("No", "否")
                }
              />
      ...
      <Text size="sm" tone="tertiary" style={taskLoopReaderCopyStyle}>
        {accounting?.mode === "effort-proxy"
          ? taskLoopCopy(
              "Active time and model-session counts are effort proxies; exact token or credit savings are unavailable.",
              "活跃时间和模型会话数仅代表投入；目前无法精确计算 token 或 credit 节省。"
            )
          : taskLoopCopy(
              "Usage totals describe observed activity, not counterfactual savings.",
              "用量只描述已观察活动，不代表反事实节省。"
            )}
        {outcomeReview && !outcomeReview.comparableModelOutcomeEvidence
          ? taskLoopCopy(
              " Model outcomes need a controlled A/B before comparison.",
              " 模型效果需要通过受控 A/B 后才能比较。"
            )
          : ""}
      </Text>
    </Stack>
...
function usageTrendLeader(series) {
  return series.reduce(
    (leader, row) =>
      Number(row.total ?? 0) > Number(leader?.total ?? -1) ? row : leader,
    null
  )
}
      ...
      <CardBody>
        <Stack gap={14}>
          ...
          {leader ? (
            <Stack gap={2}>
              <Text weight="semibold">
                {usageSeriesLabel(leader.name)} ·{" "}
                {formatUsageNumber(leader.total)} {totalLabel}
              </Text>
              <Text size="sm" tone="tertiary">
                {leaderDescription}
              </Text>
    ...
    <Stack gap={12}>
      ...
      >
        <TaskLoopUsageTrend
          ...
          leaderDescription={taskLoopCopy(
            "Most observed model in this window",
            "当前时间窗口内记录最多的模型"
          )}
          series={modelSeries}
          categories={categories}
        ...
        <TaskLoopUsageTrend
          ...
          leaderDescription={taskLoopCopy(
            "Most observed Skill in this window",
            "当前时间窗口内记录次数最多的 Skill"
          )}
          series={skillSeries}
          categories={categories}
        />
...
function taskLoopSessionInsightConfidence(row) {
  return (
    list(row?.labels)
      .map(textValue)
      .find((value) => ["High", "Medium", "Low"].includes(value)) ?? ""
  )
}
  ...
  return [
    {
      ...
      render: (row) => (
        <Text size="sm" weight="semibold">
          {taskLoopSessionInsightTitle(row.id)}
        </Text>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return confidence ? (
          <Tag size="sm" tone={confidenceTone(confidence)}>
...
function taskLoopSessionInsightDetail(row) {
  ...
  return (
    ...
    >
      <Stack gap={7}>
        <Text size="sm" tone="tertiary">
          {taskLoopCopy("Insight ID", "洞察 ID")}: {row.id} ·{" "}
          {taskLoopCopy("Status", "状态")}: {row.status ?? "—"} ·{" "}
          {taskLoopCopy("Kind", "类型")}: {row.kind ?? "—"} ·{" "}
          {taskLoopCopy("Model", "模型版本")}: {row.modelVersion ?? "—"}
        </Text>
        {list(row.labels).length ? (
          <Row gap={5} wrap>
            {row.labels.map((label) => (
              <Tag key={label} size="sm" tone="neutral">
                {label}
              </Tag>
            ))}
          </Row>
        ...
        {evidenceRefs.length ? (
          <EvidenceReferenceList items={evidenceRefs} />
        ) : (
          <Text size="sm" tone="tertiary">
            {taskLoopCopy(
              "No raw evidence references recorded.",
              "未记录原始证据引用。"
            )}
          </Text>
        )}
...
function taskLoopRepresentativeSessionInsights(entries) {
  ...
  const remaining = entries
    .filter(
      (entry) =>
        list(entry?.evidenceRefs).length > 0 && !preferred.includes(entry)
    )
    .sort(
      (left, right) =>
        list(right.evidenceRefs).length - list(left.evidenceRefs).length
    )
            ...
            <Card key={row.id} size="sm">
              <CardBody style={{ padding: 12, height: 142 }}>
                <Stack gap={8} style={{ height: "100%" }}>
                  <Row gap={7} align="center" style={{ minWidth: 0 }}>
                    <Tag size="sm" tone={rowTones[index]}>
                      {index + 1}
                    </Tag>
...
function TaskLoopFindingDialog({ row }) {
  const expectedOutput = list(row.expectedOutput).filter((item) =>
    textValue(item)
  )
  ...
  return (
    <Dialog
      trigger={
        <Button variant="ghost">
          {taskLoopCopy("View details", "查看详情")}
        </Button>
      }
    ...
    >
      <Stack gap={16}>
        <Row gap={6} align="center" wrap>
          <Tag size="sm" tone={severityTone(row.severity)}>
            {severityLabel(row.severity)}
          </Tag>
          {dimensionRefs.map((dimensionRef) => (
            <Tag key={dimensionRef} size="sm" tone="neutral">
              {taskLoopDimensionLabel(dimensionRef)}
            </Tag>
          ))}
        </Row>
        <Stack gap={6} style={taskLoopReaderCopyStyle}>
          <Text size="sm" weight="semibold" tone="tertiary">
            {taskLoopCopy("Cause", "原因")}
          </Text>
          <Text size="sm" tone="secondary" style={{ lineHeight: 1.6 }}>
            {textValue(row.reason) ||
              taskLoopCopy("No cause was recorded.", "未记录原因。")}
          </Text>
        </Stack>
        <Divider style={{ margin: "2px 0" }} />
        <Stack gap={8} style={taskLoopReaderCopyStyle}>
          <Text size="sm" weight="semibold" tone="tertiary">
            {taskLoopCopy("Expected Output", "预期结果")}
          </Text>
          {expectedOutput.length ? (
            expectedOutput.map((output, index) => (
              <Row
                key={`${row.id}-dialog-output-${index}`}
                gap={8}
                align="start"
              >
                <Tag size="sm" tone="neutral">
            ...
            <Text size="sm" tone="tertiary">
              {taskLoopCopy(
                "No expected output was recorded.",
                "未记录预期结果。"
              )}
            </Text>
          )}
...
function TaskLoopFindingCards({ findings }) {
  return (
    <Grid columns={3} minColumnWidth={300} gap={8} align="stretch">
      {findings.map((row) => (
        <TaskLoopFindingCard key={row.id} row={row} />
      ))}
    </Grid>
  )
}
...
function TaskLoopSuggestionDialog({ row }) {
  ...
  return (
    <Dialog
      trigger={
        <Button variant="ghost">
          {taskLoopCopy("Review suggestion", "查看建议")}
        </Button>
      }
      title={row.title ?? row.id}
      closeLabel={taskLoopCopy("Close", "关闭")}
    ...
    >
      <Stack gap={16}>
        <Row gap={6} align="center" wrap>
          <Tag size="sm" tone="info">
            {taskLoopSuggestionKindLabel(row.kind)}
          </Tag>
          <Tag size="sm" tone={confidenceTone(row.confidence)}>
            {confidenceLabel(row.confidence)}
          </Tag>
        </Row>
        <Stack gap={6} style={taskLoopReaderCopyStyle}>
          <Text size="sm" weight="semibold" tone="tertiary">
            {taskLoopCopy("Why this is worth trying", "为什么值得尝试")}
          </Text>
          <Text size="sm" tone="secondary" style={{ lineHeight: 1.6 }}>
            {row.reason}
          </Text>
        ...
        {prerequisites.length ? (
          <Stack gap={6}>
            <Text size="sm" weight="semibold" tone="tertiary">
              {taskLoopCopy("Prerequisites", "前置条件")}
            </Text>
            {prerequisites.map((item, index) => (
              <Row
                key={`${row.id}-prerequisite-${index}`}
                gap={8}
                align="start"
              >
                <Tag size="sm" tone="neutral">
...
function TaskLoopSuggestionCards({ suggestions }) {
  return (
    <Grid columns={3} minColumnWidth={280} gap={8} align="stretch">
      {suggestions.map((row) => (
        <TaskLoopSuggestionCard key={row.id} row={row} />
      ))}
    </Grid>
  )
}
...
function taskLoopDeliveryOutcomeLabel(boundary) {
  ...
  const levels = list(boundary?.deliveryEvidenceLevels)
    .map(textValue)
    .filter(Boolean)
  if (
    !levels.length ||
    levels.every((value) =>
      ["none", "unobserved", "not-observed"].includes(value.toLowerCase())
    )
  ) {
...
function taskLoopCoverageFraction(value, analyzedField, eligibleField) {
  if (
    !value ||
    !Object.hasOwn(value, analyzedField) ||
    !Object.hasOwn(value, eligibleField)
  )
    return null
  const analyzed = Number(value[analyzedField])
  const eligible = Number(value[eligibleField])
  return Number.isInteger(analyzed) &&
    analyzed >= 0 &&
...
function taskLoopMeasurementBoundaryText(selection, usageSelection) {
  const sample = taskLoopCoverageFraction(
    selection,
    "analyzedCount",
    "eligibleCount"
  )
  const activity = taskLoopCoverageFraction(
    usageSelection,
...
function taskLoopSelectionDetailText(selection) {
  const coverage = taskLoopCoverageFraction(
    selection,
    "analyzedCount",
    "eligibleCount"
  )
...
function taskLoopTaskEvidenceText(coverage) {
  ...
  if (!fields.every((field) => Object.hasOwn(coverage, field)))
    return taskLoopCopy("Not supplied", "未提供")
  const [episodes, edited, closed, recovered] = fields.map((field) =>
    Number(coverage[field])
  )
  if (
    ![episodes, edited, closed, recovered].every(
...
function taskLoopLearningDetailText(learning) {
  if (
    !Object.hasOwn(learning, "state") &&
    !Array.isArray(learning.interventions)
  ) {
    return taskLoopCopy("Not supplied", "未提供")
  }
  return taskLoopCopy(
...
function TaskLoopEvidenceDetails({
  usage,
  boundary,
  manifest,
  selection,
    ...
    <Stack gap={14}>
      {hasDetailedBoundary ? (
        <Stack gap={7} style={taskLoopReaderCopyStyle}>
          <Text weight="semibold">
            {taskLoopCopy("Sampling and provenance", "抽样与来源")}
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Session selection", "会话抽样")}:{" "}
            {taskLoopSelectionDetailText(selection)}
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Task evidence", "任务证据")}:{" "}
            {taskLoopTaskEvidenceText(coverage)}
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Delivery outcomes", "交付结果")}:{" "}
            {taskLoopDeliveryOutcomeLabel(boundary)}.
          </Text>
          <Text size="sm" tone="secondary">
            {taskLoopCopy("Learning comparison", "学习循环比较")}:{" "}
            {taskLoopLearningDetailText(learning)}
          </Text>
          {textValue(learning.summary) ? (
            <Text size="sm" tone="secondary">
              {learning.summary}
            </Text>
          ) : null}
      ...
      <Grid columns={3} minColumnWidth={260} gap={12} align="start">
        <UsageStatRow
          label={taskLoopCopy("Evidence mode", "证据模式")}
          value={report.summary?.evidenceMode ?? "—"}
        />
        <UsageStatRow
          label={taskLoopCopy("Platform", "平台")}
          value={manifest.platform ?? "—"}
        />
        <UsageStatRow
          label={taskLoopCopy("Adapter version", "适配器版本")}
          value={manifest.adapterVersion ?? "—"}
        />
        <UsageStatRow
          label={taskLoopCopy("Manifest schema", "清单版本")}
          value={manifest.schemaVersion ?? "—"}
        />
        <UsageStatRow
          label={taskLoopCopy("Source fingerprint", "来源指纹")}
          value={manifest.sourceFingerprint ?? "—"}
        />
        <UsageStatRow
          label={taskLoopCopy("Source gaps", "来源缺口")}
          value={
            Array.isArray(boundary.sourceGaps)
              ? boundary.sourceGaps.length
              : "—"
          }
        />
      </Grid>
...
function TaskLoopEvidenceBoundary({ usage }) {
  ...
  const samplingConfidence =
    textValue(selection.confidence) || taskLoopCopy("Not supplied", "未提供")
  const sourceGapValue = Array.isArray(boundary.sourceGaps)
    ? boundary.sourceGaps.length
    : "—"

  return (
    <Stack gap={12}>
      <H2 style={{ fontSize: 20, lineHeight: 1.35 }}>
      ...
      <Grid columns={3} minColumnWidth={180} gap={8} align="stretch">
        <TaskLoopEvidenceFact
          label={taskLoopCopy("Sampling confidence", "抽样可信度")}
          value={samplingConfidence}
          tone={confidenceTone(selection.confidence)}
        />
        <TaskLoopEvidenceFact
          label={taskLoopCopy("Source gaps", "来源缺口")}
          value={sourceGapValue}
          tone={
            Array.isArray(boundary.sourceGaps) && boundary.sourceGaps.length
              ? "warning"
              : "neutral"
          }
        />
        <TaskLoopEvidenceFact
          label={taskLoopCopy("Delivery outcome", "交付结果")}
          value={taskLoopDeliveryOutcomeLabel(boundary)}
        />
      </Grid>
      ...
      <CollapsibleSection
        ...
        title={
          <Row gap={12} align="center" wrap>
            <Text weight="semibold">
              {taskLoopCopy(
                "View measurement and model details",
                "查看计量与模型明细"
              )}
            </Text>
            <Text size="sm" tone="secondary">
              {taskLoopCopy(
                "Response accounting, model detail, and sampling method",
                "响应计量、模型明细与抽样方法"
              )}
            </Text>
          </Row>
        ...
        trailing={
          <Text size="sm" tone="secondary">
            {taskLoopMeasurementModelLabel(usage, modelUsage)}
          </Text>
        }
...
function TaskLoopReport() {
  ...
  const findings = list(report.findings).sort(
        ...
        {suggestions.length ? (
          <>
            ...
            <Row justify="space-between" align="center" wrap gap={8}>
              <Stack gap={3}>
                <Text weight="semibold">
                  {taskLoopCopy("Suggestions", "建议")}
                </Text>
                <Text
                  size="sm"
                  tone="secondary"
                  style={taskLoopReaderCopyStyle}
                >
                  {taskLoopCopy(
              ...
              <Tag size="sm" tone="neutral">
                {suggestions.length} {taskLoopCopy("suggestions", "条建议")}
              </Tag>
            </Row>
