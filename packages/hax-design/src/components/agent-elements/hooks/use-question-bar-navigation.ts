 ...
 */
export function useQuestionBarNavigation(
  questionBar: QuestionBarData | undefined
  ...
  const hasExternalQuestionNavigation = Boolean(
    questionBar?.onPreviousQuestion || questionBar?.onNextQuestion
  )
  const questionIndex = hasExternalQuestionNavigation
    ? (questionBar?.questionIndex ?? 1)
    : questionBarIndex
