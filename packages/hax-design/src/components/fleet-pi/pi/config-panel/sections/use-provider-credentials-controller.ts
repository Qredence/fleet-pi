...
export function useProviderCredentialsController({
  ...
  // providers (`custom+…`) and the custom-provider template row are editable
  // credential entries even though they are not in the static catalog.
  ...
  const handleSave = async (providerId: string) => {
    ...
    const isCustomEditor =
      providerId === CUSTOM_PROVIDER_PICKER_ID || isCustomProviderId(providerId)
    ...
    if (isCustomEditor) {
      ...
      try {
        await onUpdateProvider({
          ...
          api,
          models: modelList,
          ...(providerId === CUSTOM_PROVIDER_PICKER_ID
  ...
  const canSave = useMemo(() => {
    ...
    if (
      editingProvider === CUSTOM_PROVIDER_PICKER_ID ||
      isCustomProviderId(editingProvider)
    ) {
  ...
  const keywords =
    provider.id === CUSTOM_PROVIDER_PICKER_ID || isCustomProviderId(provider.id)
      ...
      : isOccProviderId(provider.id)
        ...
        : ""
  const haystack = [provider.name, provider.envVarName, provider.id, keywords]
    .join(" ")
    .toLowerCase()
