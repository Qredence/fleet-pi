...
- Memory snippet injection is currently static: `extractMemorySnippets` takes the first 4 bullet-list lines per canonical file; `formatMemorySnippets` surfaces the first 3 per file (max 15 snippets per turn). Selection is order-based, not prompt-aware.
