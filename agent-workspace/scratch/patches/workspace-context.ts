...
export default function workspaceContextExtension(pi: ExtensionAPI) {
  ...
  pi.on("context", async (event, ctx) => {
    ...
    const index = await readProjectMemoryIndex(cwd)
    parts.push(formatProjectMemoryForStartupContext(index, lastUserPrompt))

    const updatedMessages = messages.map((msg, i) => {
      if (i === lastWorkspaceContextIndex) {
        return {
          ...msg,
          content: parts.join("\n"),
        }
    ...
    const filtered = updatedMessages.filter((msg, i) => {
